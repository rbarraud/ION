...
# unlike the original Plasma code, this one does not check all the test success 
# conditions. Instead, for some tst cases it performs the operations to be
# tested and relies on you to compare the logs from the logic simulation and 
# the software simulation.
# Tests that work this way have been commented with this tag: "@log"
#
#-------------------------------------------------------------------------------
# @note1: Hardware interrupt simulation 
#   
#   FIXME update this to 32r2 arch.
#   TODO Unimplementd yet.
#
...
################################################################################
    ...
    .set TEST_BAD_OPCODES, 0            # Test opcode emulation trap
    .set TEST_TRAP_DELAY_EMU, 0         # Test emu of bad opcodes in delay slots
    .set TEST_HW_INTS, 0                # Test HW interrupts (@note1)
    
    .set USE_CACHE, 0                   # Initialize and enable cache
...
entry:
    ...
    # and we will modify registers $k0 and $k1 only, so we will not save 
    # any registers (there's no stack anyway).
InterruptVector:
    ...
    bne     $k0,$k1,trap_invalid # if it was NOT, then it's a faulty cause code.
    nop
...
trap_syscall:
    ...
    andi    $k0,$k0,0x1f
    addi    $k0,$k0,-8
    bnez    $k0,trap_syscall_quit
    nop
    addi    $4,$2,34
trap_syscall_quit:
    eret
    ori     $4,$0,0x99          #should't be executed, will be caugh if it does
...
Reset:
    # Initialize all the regs that we'll be using as global data.
    ...
    # Set up the simulated wait state conig register. The whole test will run 
    # with the same configuration.
    li      $k0,0x20010000
    ori     $k1,$0,DATA_WAIT_STATES
    sw      $k1,0x34($k0)
    
    # Read some COP0 regs so they come out in the log and their reset value 
    # can be verified.
    mfc0    $k0,$12
    mfc0    $k0,$13
    # FIXME significant COP0 registers missing
    
    # Conditionally set up the cache and do all the TB with it enabled.
    .ifgt   USE_CACHE
    jal     setup_cache
    nop
    .else
    # FIXME refactor cache control
    #ori     $k0,$zero,0x0
    ...
    ori     $a0,$0,0x34
    mfc0    $k1,$13             # verify COP* in user mode triggers trap (@log)
    nop
    mfc0    $k1,$12             # verify COP* in user mode triggers trap (@log)
    # The two above COP0s should trigger a trap. The 1st one tests the delay in
    # entering user mode (that is, we should enter with no delay).
    # None of them should modify COP0[Status].
    
    # The rest of the tests proceed in user mode (not that there's much 
    # difference...)
    

    PUTNL                       # clear a few lines in the console
    PUTNL
...
break_jump_test1:
    ...
    # FIXME @hack4 test BREAK in jump delay slot
...
break_continue:
    ...
    .ifgt TEST_SYSCALL
    #q: SYSCALL
    ori     $2,$0,'q'       # check if it jumps to trap vector and comes back
    sb      $2,0($20)
    ori     $5,$0,'A'
    ori     $2,$0,0
    ori     $4,$0,0
    ...
    ori     $2,$0,20
    syscall 0               # check if load instruction is aborted
    lb      $2,16($2)       
    addi    $4,$4,-54
    bnez    $4,syscall_fail
    nop
...
syscall_jump_test1:
    ...
    # FIXME @hack4 test syscall in delay slot
...
setup_cache:
    li      $a0,0x00010000      # Enable I-cache line invalidation
    mtc0    $a0,$12
...
inv_d_cache_loop:
    ...
    li      $a1,0x00020010      # Leave in UM with cache enabled
    jr      $ra
    mtc0    $a1,$12       
    .set    reorder
    .end    entry
